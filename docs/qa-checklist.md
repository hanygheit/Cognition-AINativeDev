...
`test:pdf` starts the production server, calls `/api/generate-pdf`, asserts exactly 18 pages, and writes `artifacts/Hany-Saad-AI-Native-Development-CognitionX-Egypt-2026.pdf`.

## Routes

- [ ] `/` redirects to `/present/1`
- [ ] `/present/1` … `/present/18` all render
- [ ] `/present/0`, `/present/19`, `/present/abc` return 404 rather than crashing
- [ ] `/overview` renders 18 thumbnails, each linking to its slide
- [ ] `/presenter` renders current slide, next preview, notes, timers
- [ ] `/pdf-render` renders 18 stacked pages with no controls
...
- [ ] Bounds are respected at slides 1 and 18 (no dead route)
...
- [ ] Fonts are embedded; local images resolve; the LinkedIn codes on slides 1 and 18 and repository code on slide 18 decode correctly
