...
- One 16:9 slide per page, 18 pages, `13.333in × 7.5in`, zero margins.
...
| Surface | `src/app/pdf-render/page.tsx` → `PdfDocument` | Renders all 18 slides stacked, sets `data-pdf-ready` after fonts and images settle |
| Test | `scripts/test-pdf.mjs` | Boots the production server, calls the endpoint, asserts 18 pages, decodes all three QR placements, writes `artifacts/` |
...
2. No `IntersectionObserver`, viewport, or scroll-driven reveals — all 18 slides are laid out at once.
...
5. Page break control lives in `.pdf-slide { break-after: page }` plus `@page { size: 13.333in 7.5in; margin: 0 }`. Changing either can produce duplicate or missing pages — always re-run `npm run test:pdf`.
...
| Page count ≠ 18 | A slide overflowed `7.5in` or a break rule changed |
