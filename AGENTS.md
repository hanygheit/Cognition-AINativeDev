...
- Preserve the locked order and core content of exactly 18 slides. Do not add, remove, reorder, or materially rewrite slides without explicit approval.
...
- Support stable routes `/present/1` through `/present/18`, plus `/presenter`, `/overview`, `/pdf-render`, and `/api/generate-pdf`. Reject or redirect invalid slide numbers safely.
...
- Preserve the filename `Hany-Saad-AI-Native-Development-CognitionX-Egypt-2026.pdf` and verify that the result has exactly 18 pages.
...
- Keep speaker identity and portrait usage limited to slides 1 and 18. Use the supplied portrait only; otherwise retain a tasteful placeholder.
